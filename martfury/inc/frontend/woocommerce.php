...
class Martfury_WooCommerce {
	...
	function __construct() {
		...
		add_action( 'woocommerce_after_shop_loop_item_title', array( $this, 'product_add_to_cart' ), 140 );
		...
		if ( martfury_get_option( 'product_loop_hover' ) == '2' ) {
			add_action( 'martfury_woo_after_shop_loop_item', array( $this, 'open_product_details_hover' ), 20 );
			add_action( 'martfury_woo_after_shop_loop_item', array( $this, 'product_add_to_cart' ), 30 );
			add_action( 'martfury_woo_after_shop_loop_item', array( $this, 'close_product_details_hover' ), 40 );
		}
		...
		if ( ! empty( $icons ) && $show_icons ) {
			...
			foreach ( $icons as $icon ) {
				if ( 'cart' == $icon ) {
					...
					if ( martfury_get_option( 'product_loop_hover' ) != '2' && function_exists( 'woocommerce_template_loop_add_to_cart' ) ) {
	...
	function product_instagram_photos() {
		...
		if ( is_wp_error( $instagram_array ) ) {
			...
			foreach ( $instagram_array as $instagram_item ) {
				...
				$image_link = $instagram_item['images']['thumbnail'];
